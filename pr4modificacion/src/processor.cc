...
void Processor::run(std::string line, int& lineNum) {
  ...
  } else if (std::regex_match(line, matches, std::regex(includeExp))) {
    Token token(Token_type::INCLUDE, "#include", lineNum);
    include_.push_back(token);

  } else if (std::regex_match(line, matches, std::regex(ifndefExp))) {
    Token token(Token_type::IFNDEF, "#ifndef", lineNum);
    ifndef_.push_back(token);
...
void Processor::printData(void) {
  ...
  outputFile << "INCLUDE:" << std::endl;
  for (unsigned i = 0; i < include_.size(); i++) {
    outputFile << include_[i].to_String() << std::endl;
  }
  outputFile << "IFNDEF:" << std::endl;
  for (unsigned i = 0; i < ifndef_.size(); i++) {
    outputFile << ifndef_[i].to_String() << std::endl;
  }
